...
const MCP_ACTION_RESULT_HOOK_COMMAND = 'npx -y @getmarrow/mcp@3.9.56 hook';
const SDK_INTEGRITY = 'sha512-aq8g3srJ9EFZVvskSQVE9MLUS8SSkvx3xY3Mr9G0ZkpKxiiDBQaa3r2pb/u74BYGVvJRP6EP/oV/D9jbKb2lBA==';

function writeSdkLock(root, declaredSpec = '^3.7.55') {
  fs.writeFileSync(path.join(root, 'package-lock.json'), JSON.stringify({
    ...
    packages: {
      ...
      'node_modules/@getmarrow/sdk': {
        version: '3.7.55',
        resolved: 'https://registry.npmjs.org/@getmarrow/sdk/-/sdk-3.7.55.tgz',
...
test('Claude native-hook activation certifies pre-action, result, and session-end coverage', () => {
  ...
  try {
    ...
    assert.equal(profile.adapter_version, '3.9.56');
    ...
    const canonicalFingerprint = crypto.createHash('sha256').update(JSON.stringify({
      ...
      descriptors: {
        prompt: [{ matcher: null, command: 'npx -y @getmarrow/mcp@3.9.56 context-hook', timeout: null }],
        pre_action: [{ matcher: 'Bash|Edit|Write|MultiEdit|mcp__(?!marrow_).*', command: 'npx -y @getmarrow/mcp@3.9.56 pre-action-hook', timeout: null }],
        action_result_success: [{ matcher: 'Bash|Edit|Write|MultiEdit|mcp__(?!marrow_).*', command: 'npx -y @getmarrow/mcp@3.9.56 hook', timeout: null }],
        action_result_failure: [{ matcher: 'Bash|Edit|Write|MultiEdit|mcp__(?!marrow_).*', command: 'npx -y @getmarrow/mcp@3.9.56 hook', timeout: null }],
        session_end: [{ matcher: null, command: 'npx -y @getmarrow/mcp@3.9.56 session-hook', timeout: null }],
      },
    ...
    assert.match(settings, /getmarrow\/mcp@3\.9\.56 hook/);
  } finally {
...
test('Claude setup replaces old Marrow hooks without duplicate execution', () => {
  ...
  try {
    ...
    assert.ok(commandCounts.every((hook) => hook.command.includes('@getmarrow/mcp@3.9.56')));
...
test('custom SDK activation requires both dependency and exact generated runtime', () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({ dependencies: { '@getmarrow/sdk': '^3.7.55' } }));
    const moduleDir = path.join(root, 'node_modules', '@getmarrow', 'sdk');
    fs.mkdirSync(moduleDir, { recursive: true });
    fs.writeFileSync(path.join(moduleDir, 'package.json'), JSON.stringify({ name: '@getmarrow/sdk', version: '3.7.55' }));
    writeSdkLock(root);
...
test('custom SDK activation rejects npm aliases even when version and installed name are forged', () => {
  ...
  try {
    fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({
      dependencies: { '@getmarrow/sdk': 'npm:untrusted-sdk@3.7.55' },
    }));
    const moduleDir = path.join(root, 'node_modules', '@getmarrow', 'sdk');
    fs.mkdirSync(moduleDir, { recursive: true });
...
test('custom SDK activation rejects override impersonation despite forged installed metadata', () => {
  ...
  try {
    fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({
      dependencies: { '@getmarrow/sdk': '^3.7.55' },
      overrides: { '@getmarrow/sdk': 'npm:untrusted-sdk@3.7.55' },
    }));
    writeSdkLock(root);
    const moduleDir = path.join(root, 'node_modules', '@getmarrow', 'sdk');
    fs.mkdirSync(moduleDir, { recursive: true });
