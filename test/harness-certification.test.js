...
const {
  ...
  NATIVE_HOOK_MATCHER: SOURCE_NATIVE_HOOK_MATCHER,
  CURSOR_NATIVE_HOOK_MATCHER: SOURCE_CURSOR_NATIVE_HOOK_MATCHER,
...
const MCP_ACTION_RESULT_HOOK_COMMAND = 'npx -y --package=@getmarrow/mcp@3.9.89 marrow-mcp hook';
...
test('native matchers include supported read, search, and status tools without matching official Marrow tools', () => {
  assert.equal(SOURCE_NATIVE_HOOK_MATCHER, NATIVE_HOOK_MATCHER);
  assert.equal(SOURCE_CURSOR_NATIVE_HOOK_MATCHER, CURSOR_NATIVE_HOOK_MATCHER);
  const claude = new RegExp(`^(?:${NATIVE_HOOK_MATCHER})$`);
  for (const tool of ['Read', 'Glob', 'Grep', 'Search', 'WebSearch', 'functions.exec']) assert.equal(claude.test(tool), true, tool);
  assert.equal(claude.test('mcp__marrow__marrow_agent_status'), false);
  const cursor = new RegExp(`^(?:${CURSOR_NATIVE_HOOK_MATCHER})$`);
  for (const tool of ['Read', 'Glob', 'Grep', 'Search', 'WebSearch', 'List']) assert.equal(cursor.test(tool), true, tool);
  assert.equal(cursor.test('MCP:marrow_agent_status'), false);
  const gemini = new RegExp(GEMINI_NATIVE_HOOK_MATCHER);
  for (const tool of ['read_file', 'read_many_files', 'glob', 'grep_search', 'list_directory', 'get_file_info', 'web_search']) assert.equal(gemini.test(tool), true, tool);
...
const SDK_INTEGRITY = 'sha512-5BiV1P0J1NMVdgjqxqULfM+zGrTjHBuWm1amKZWba4yW6C3t10VlhIlj/A+4YKbKqvyZe8gXgUwW+VrYDbVQrQ==';
...
test('Cursor hooks reconcile exact native events, preserve unrelated entries, and stay byte-idempotent', () => {
  ...
  try {
    ...
    const exact = (event, suffix, matcher, timeout) => {
      ...
      assert.match(entry.command, /@getmarrow\/mcp@3\.9\.89/);
      return entry;
    };
...
test('Cline hooks install exact executable non-blocking scripts and remain byte-idempotent', () => {
  ...
  try {
    ...
    assert.match(pre, /@getmarrow\/mcp@3\.9\.89 marrow-mcp cline-pre-action-hook/);
    assert.match(pre, /"cancel":true/);
    assert.match(pre, /JSON\.parse/);
    assert.match(post, /@getmarrow\/mcp@3\.9\.89 marrow-mcp cline-hook/);
    assert.match(post, /\|\| :/);
    assert.match(cancel, /@getmarrow\/mcp@3\.9\.89 marrow-mcp cline-session-hook/);
    assert.match(cancel, /\|\| :/);
...
test('Windsurf reconciles exact native hooks, preserves unrelated config, fails closed only before actions, and is byte-idempotent', () => {
  ...
  try {
    ...
    for (const eventName of WINDSURF_EVENTS) {
      ...
      assert.match(marrow[0].command, /@getmarrow\/mcp@3\.9\.89/);
    }
...
test('Gemini CLI reconciles exact native groups, validates decisions, keeps neutral closeout, and is byte-idempotent', () => {
  ...
  try {
    ...
    for (const [eventName, name, entrypoint, timeout, matcher] of expected) {
      ...
      assert.match(marrowHandlers[0].command, new RegExp(`@getmarrow/mcp@3\\.9\\.89 marrow-mcp ${entrypoint}`));
...
test('fresh Grok installs create one private managed global hook and remain byte-idempotent', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'marrow-harness-grok-fresh-'));
  try {
    fs.writeFileSync(path.join(root, 'package.json'), '{}\n');
    fs.mkdirSync(path.join(root, '.grok'), { mode: 0o700 });
    const detection = detectEnvironment(root, { ...process.env, HOME: root });
    const hookPath = path.join(root, '.grok', 'hooks', 'marrow.json');
    assert.equal(fs.existsSync(hookPath), false);
    const changes = applyPlan(buildPlan(detection, { mode: 'mcp' }), { yes: true, dryRun: false, doctor: false });
    assert.equal(changes.find((entry) => entry.label === 'Grok native hooks').applied, true);
    assert.equal(fs.statSync(hookPath).mode & 0o777, 0o600);
    const first = fs.readFileSync(hookPath);
    const settings = JSON.parse(first);
    assert.match(JSON.stringify(settings), /@getmarrow\/mcp@3\.9\.89/);
    ...
    const second = applyPlan(buildPlan(detectEnvironment(root, { ...process.env, HOME: root }), { mode: 'mcp' }), { yes: true, dryRun: false, doctor: false });
    assert.deepEqual(fs.readFileSync(hookPath), first);
    assert.equal(second.find((entry) => entry.label === 'Grok native hooks').already_present, true);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});

test('Grok install preserves unmanaged files and rejects symlinked or out-of-home hook targets', () => {
  for (const kind of ['unmanaged', 'symlink', 'outside']) {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), `marrow-harness-grok-${kind}-`));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'marrow-harness-grok-outside-'));
    try {
      fs.writeFileSync(path.join(root, 'package.json'), '{}\n');
      const hooks = path.join(root, '.grok', 'hooks');
      fs.mkdirSync(hooks, { recursive: true, mode: 0o700 });
      const hookPath = path.join(hooks, 'marrow.json');
      const ownerBytes = '{"owner":true}\n';
      if (kind === 'unmanaged') fs.writeFileSync(hookPath, ownerBytes, { mode: 0o600 });
      if (kind === 'symlink') {
        const outsideFile = path.join(outside, 'owner.json');
        fs.writeFileSync(outsideFile, ownerBytes, { mode: 0o600 });
        fs.symlinkSync(outsideFile, hookPath);
      }
      const detection = detectEnvironment(root, { ...process.env, HOME: root });
      if (kind === 'outside') detection.paths.grokHooks = path.join(outside, 'marrow.json');
      const plan = buildPlan(detection, { mode: 'mcp' });
      if (kind === 'unmanaged') {
        const changes = applyPlan(plan, { yes: true, dryRun: false, doctor: false });
        const conflict = changes.find((entry) => entry.label === 'Grok native hooks');
        assert.equal(conflict.hook_conflict, true);
        assert.equal(conflict.applied, false);
        assert.equal(fs.readFileSync(hookPath, 'utf8'), ownerBytes);
      } else {
        assert.throws(
          () => applyPlan(plan, { yes: true, dryRun: false, doctor: false }),
          /unsafe owner file|outside the direct owner hook path/,
        );
      }
    } finally {
      fs.rmSync(root, { recursive: true, force: true });
...
test('Claude native-hook configuration records local completeness without proving runtime coverage', () => {
  ...
  try {
    ...
    assert.equal(profile.capability_level, 'native_hooks');
    assert.equal(profile.adapter_version, '3.9.89');
    ...
    const canonicalFingerprint = crypto.createHash('sha256').update(JSON.stringify({
      ...
      descriptors: {
        prompt: [{ matcher: null, command: 'npx -y --package=@getmarrow/mcp@3.9.89 marrow-mcp context-hook', timeout: null }],
        pre_action: [{ matcher: 'Bash|Edit|Write|MultiEdit|Read|Glob|Grep|Search|WebSearch|Task|functions\\.(?!mcp__marrow__marrow_).*|mcp__(?!marrow__marrow_).*', command: 'npx -y --package=@getmarrow/mcp@3.9.89 marrow-mcp pre-action-hook', timeout: null }],
        action_result_success: [{ matcher: 'Bash|Edit|Write|MultiEdit|Read|Glob|Grep|Search|WebSearch|Task|functions\\.(?!mcp__marrow__marrow_).*|mcp__(?!marrow__marrow_).*', command: 'npx -y --package=@getmarrow/mcp@3.9.89 marrow-mcp hook', timeout: null }],
        action_result_failure: [{ matcher: 'Bash|Edit|Write|MultiEdit|Read|Glob|Grep|Search|WebSearch|Task|functions\\.(?!mcp__marrow__marrow_).*|mcp__(?!marrow__marrow_).*', command: 'npx -y --package=@getmarrow/mcp@3.9.89 marrow-mcp hook', timeout: null }],
        session_end: [{ matcher: null, command: 'npx -y --package=@getmarrow/mcp@3.9.89 marrow-mcp session-hook', timeout: null }],
      },
    ...
    assert.match(settings, /getmarrow\/mcp@3\.9\.89.*marrow-mcp hook/);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test('Claude setup replaces old Marrow hooks without duplicate execution', () => {
  ...
  try {
    ...
    assert.ok(commandCounts.every((hook) => hook.command.includes('@getmarrow/mcp@3.9.89')));
...
test('Codex hooks reconcile exact native events, preserve unrelated entries, and stay byte-idempotent', () => {
  ...
  try {
    ...
    const exact = (event, suffix, matcher, timeout) => {
      ...
      assert.match(hook.command, /@getmarrow\/mcp@3\.9\.89/);
      return hook;
    };
  ...
  try {
    ...
    fs.writeFileSync(path.join(root, '.claude', 'settings.json'), JSON.stringify({
      hooks: {
        ...
        PreToolUse: [{ matcher: 'Bash|Edit|Write|MultiEdit|Read|Glob|Grep|Search|WebSearch|Task|functions\\.(?!mcp__marrow__marrow_).*|mcp__(?!marrow__marrow_).*', hooks: [{ type: 'command', command: 'echo npx -y @getmarrow/mcp pre-action-hook' }] }],
        PostToolUse: [{ matcher: 'Bash|Edit|Write|MultiEdit|Read|Glob|Grep|Search|WebSearch|Task|functions\\.(?!mcp__marrow__marrow_).*|mcp__(?!marrow__marrow_).*', hooks: [{ type: 'command', command: 'echo npx -y @getmarrow/mcp hook' }] }],
        PostToolUseFailure: [{ matcher: 'Bash|Edit|Write|MultiEdit|Read|Glob|Grep|Search|WebSearch|Task|functions\\.(?!mcp__marrow__marrow_).*|mcp__(?!marrow__marrow_).*', hooks: [{ type: 'command', command: 'echo npx -y @getmarrow/mcp hook' }] }],
...
test('custom SDK activation requires both dependency and exact generated runtime', () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({ dependencies: { '@getmarrow/sdk': '^3.7.63' } }));
...
test('custom SDK activation rejects npm aliases even when version and installed name are forged', () => {
  ...
  try {
    fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({
      dependencies: { '@getmarrow/sdk': 'npm:untrusted-sdk@3.7.63' },
    }));
    const moduleDir = path.join(root, 'node_modules', '@getmarrow', 'sdk');
    fs.mkdirSync(moduleDir, { recursive: true });
...
test('custom SDK activation rejects override impersonation despite forged installed metadata', () => {
  ...
  try {
    fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({
      dependencies: { '@getmarrow/sdk': '^3.7.63' },
      overrides: { '@getmarrow/sdk': 'npm:untrusted-sdk@3.7.63' },
    }));
    writeSdkLock(root);
    const moduleDir = path.join(root, 'node_modules', '@getmarrow', 'sdk');
    fs.mkdirSync(moduleDir, { recursive: true });
