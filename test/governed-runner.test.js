...
test('governed proof distinguishes command observation from verified completion', () => {
  const options = { proofFile: '', profile: 'default', client: 'codex', agentId: 'agent-a' };
  const observed = defaultProof({
    options,
    action: 'deploy service',
    childCommand: ['wrangler', 'deploy'],
    exitCode: 0,
    success: true,
  });
  const verified = defaultProof({
    options,
    action: 'run tests',
  ...
  assert.equal(observed.evidence_state, 'observed_only');
  assert.equal(observed.verified_completion, false);
  assert.deepEqual(observed.checks, ['command_exit_code=0']);
  assert.equal(verified.evidence_state, 'verified');
  assert.equal(verified.verified_completion, true);
  assert.equal(JSON.stringify([observed, verified]).includes('command exit captured'), false);
});
...
test('local integration registry covers major harnesses and model CLIs', () => {
  ...
  assert.ok(mcpSetupCommands.every((command) => command.includes('--package=@getmarrow/mcp@3.9.93')));
...
test('governed runner attaches stable client attribution from env and CLI', () => {
  ...
  try {
    ...
    assert.equal(headers(envParsed.options)['X-Marrow-Package-Version'], '0.1.61');
    assert.equal(headers(envParsed.options)['X-Marrow-Install-Version'], '0.1.61');
    assert.equal(headers(envParsed.options)['X-Marrow-SDK-Version'], '3.7.63');
    assert.equal(headers(envParsed.options)['X-Marrow-MCP-Version'], '3.9.93');
...
test('detectProjectSignals finds deploy and Cloudflare project evidence', () => {
  ...
  assert.match(signals.fingerprint, /^[0-9a-f]{64}$/);
  assert.equal(JSON.stringify(signals).includes(dir), false);
});
...
test('integration coverage matrix names exact lifecycle and repair limits for every harness', () => {
  ...
  const composer = matrix.find((entry) => entry.harness === 'composer');
  ...
  assert.equal(cursor.pre_action, 'automatic_native_hook');
  assert.equal(cursor.action_result, 'automatic_native_hook');
  assert.equal(cursor.outcome_closure, 'correlated_when_determinable');
  assert.equal(cursor.proof_enforcement, 'automatic_for_protected_actions');
  assert.equal(cursor.automatic_repair, 'installer_managed_config_only');
  assert.match(cursor.limitation, /business outcome/);
  assert.equal(composer.capability_level, 'native_hooks');
  assert.equal(composer.pre_action, cursor.pre_action);
  assert.equal(composer.action_result, cursor.action_result);
  assert.equal(composer.outcome_closure, cursor.outcome_closure);
...
test('fleet panel summarizes operator-critical account state', () => {
  ...
  assert.match(panel, /Marrow client update: recommended; installed=0\.1\.35; latest=0\.1\.43; operator approval required/);
...
test('status panel makes an available update actionable without silently applying it', () => {
  ...
  assert.match(panel, /Client update: recommended; installed=0\.1\.35; latest=0\.1\.43/);
