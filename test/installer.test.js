...
test('release-candidate adapter provenance matches the exact MCP source and certified SDK package', () => {
  assert.deepEqual(ADAPTER_PROVENANCE, {
    mcp: {
      ...
      version: '3.9.89',
      source_sha: 'ff229e17419f65aeebd7fa7754dd61cbda61900d',
      integrity: 'sha512-KC/P4dStzOOfZKxSwigQE4TWCt1TBgYINQzlXrnA5jpYHRrT2Jo73vaV4FiucVPOT+xyU1uCci+bmJoC35aZ0g==',
      integrity_state: 'verified_npm_registry_metadata',
    },
    sdk: {
      package: '@getmarrow/sdk',
      version: '3.7.63',
      integrity: 'sha512-5BiV1P0J1NMVdgjqxqULfM+zGrTjHBuWm1amKZWba4yW6C3t10VlhIlj/A+4YKbKqvyZe8gXgUwW+VrYDbVQrQ==',
    },
  });
  assert.notEqual(ADAPTER_PROVENANCE.mcp.source_sha, '24a78df7fcc4cd33a79a25f571cd41782d082516');
});

function loopGuardProof() {
  return {
    pass: true,
    isolated: true,
...
test('isolated MCP loop-guard self-test uses the exact pin and never touches the owner home', () => {
  const ownerHome = tempDir();
  let invocation;
  try {
    const result = runLoopGuardSelfTest({
      loopGuardSelfTestRunner(input) {
        invocation = input;
        ...
        assert.deepEqual(input.args, ['-y', '--package=@getmarrow/mcp@3.9.89', 'marrow-mcp', 'loop-guard-self-test']);
        return { status: 0, stdout: JSON.stringify(loopGuardProof()), stderr: '' };
      },
    });
    assert.equal(result.passed, true);
    assert.equal(result.live_hook_observed, false);
    assert.equal(fs.existsSync(path.join(ownerHome, '.marrow')), false);
    assert.equal(fs.existsSync(invocation.env.HOME), false);
  } finally {
    fs.rmSync(ownerHome, { recursive: true, force: true });
  }
});

test('install reports loop guard configuration, isolated proof, and observation separately', async () => {
  const root = tempDir();
  const home = tempDir();
  ...
  try {
    ...
    const report = await install({
      ...
      loopGuardSelfTestRunner: () => ({ status: 0, stdout: JSON.stringify(loopGuardProof()), stderr: '' }),
      apiKey: '',
      baseUrl: 'https://api.getmarrow.ai',
      agentId: 'loop-guard-fixture',
      controller: false,
      processCommands: [],
      mcpRegistryMetadata: null,
    });
    assert.equal(report.loop_guard_configured, true);
    assert.equal(report.loop_guard_self_tested, true);
    assert.equal(report.loop_guard_observed, false);
    assert.equal(report.activation.profile.loop_guard_configured, true);
    assert.equal(report.activation.profile.loop_guard_self_tested, true);
    assert.equal(report.activation.profile.loop_guard_observed, false);
    assert.equal(report.doctor.loop_guard_configured, true);
    assert.equal(report.doctor.loop_guard_self_tested, true);
    assert.equal(report.doctor.loop_guard_observed, false);
  } finally {
...
test('update preserves explicit owner disable bytes and does not ensure the controller', async () => {
  const root = tempDir();
  const home = tempDir();
  const originalHome = process.env.HOME;
  process.env.HOME = home;
  try {
    ...
    const controlPath = path.join(home, '.marrow', 'control.json');
    ...
    const report = await install({
      ...
      apiKey: '',
      baseUrl: 'https://api.getmarrow.ai',
      agentId: 'owner-disabled-fixture',
      controlHome: home,
      controller: true,
    ...
    assert.deepEqual(fs.readFileSync(controlPath), before);
    assert.equal(report.local_control.enabled, false);
    assert.equal(report.activation.profile.loop_guard_enabled, false);
    assert.equal(report.controller.active, false);
  } finally {
    if (originalHome === undefined) delete process.env.HOME;
    else process.env.HOME = originalHome;
    fs.rmSync(root, { recursive: true, force: true });
...
test('doctor identifies stale and mixed MCP processes without exposing command lines', () => {
  const report = inspectMcpProcesses({ commands: [
    ...
    'npx -y --package=@getmarrow/mcp@3.9.89 marrow-mcp',
  ...
  assert.deepEqual(report.active_versions, ['2.8.0', '3.9.89']);
  ...
  assert.match(report.restart_instruction, /after the update completes.*restart.*once/i);
  assert.match(report.restart_instruction, /remain unchanged until that restart/i);
...
test('doctor ignores parent shells and sandbox wrappers that only mention MCP commands', () => {
  const report = inspectMcpProcesses({ commands: [
    '/usr/bin/bwrap --ro-bind / / /bin/bash -lc npx -y --package=@getmarrow/mcp@3.9.89 marrow-mcp setup',
...
test('doctor flags stale and mixed MCP owner configurations without exposing paths or contents', () => {
  ...
  fs.writeFileSync(current, JSON.stringify({ mcp: { command: 'npx', args: ['-y', '--package=@getmarrow/mcp@3.9.89', 'marrow-mcp'] } }));
  try {
    const report = inspectMcpConfigurations({}, { paths: [stale, current] });
    assert.equal(report.healthy, false);
    assert.equal(report.mixed_versions, true);
    assert.deepEqual(report.configured_versions, ['2.8.0', '3.9.89']);
    assert.deepEqual(report.stale_versions, ['2.8.0']);
    assert.equal(report.exact_fix, 'npx -y @getmarrow/install@latest update');
    assert.equal(report.restart_required, true);
    assert.equal(report.verification_command, 'npx -y @getmarrow/install@latest doctor --self-test');
...
test('doctor accepts a current-only MCP owner configuration', () => {
  ...
  fs.writeFileSync(current, JSON.stringify({ mcpServers: { marrow: { command: 'npx', args: ['-y', '--package=@getmarrow/mcp@3.9.89', 'marrow-mcp'] } } }));
  try {
    const report = inspectMcpConfigurations({}, { paths: [current] });
    assert.equal(report.healthy, true);
    assert.equal(report.mixed_versions, false);
    assert.deepEqual(report.configured_versions, ['3.9.89']);
    assert.equal(report.exact_fix, null);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
...
test('doctor treats stable newer minor and major MCP configurations as unverified-ahead', () => {
  const root = tempDir();
  const newerMinor = path.join(root, '.mcp.json');
  const newerMajor = path.join(root, 'claude.json');
  fs.writeFileSync(newerMinor, JSON.stringify({ mcpServers: { marrow: { args: ['--package=@getmarrow/mcp@3.10.0'] } } }));
  fs.writeFileSync(newerMajor, JSON.stringify({ mcpServers: { marrow: { args: ['--package=@getmarrow/mcp@4.0.0'] } } }));
  try {
    const report = inspectMcpConfigurations({}, { paths: [newerMinor, newerMajor] });
    assert.equal(report.healthy, false);
    assert.deepEqual(report.configured_versions, ['3.10.0', '4.0.0']);
    ...
    assert.doesNotMatch(report.exact_fix, /@getmarrow\/install@latest update|marrow-mcp setup/);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test('doctor treats the previously pinned MCP 3.9.59 as stale', () => {
  ...
  assert.equal(report.expected_version, '3.9.89');
...
test('doctor treats both immediately superseded MCP releases as stale', () => {
  for (const version of ['3.9.72', '3.9.73']) {
    const report = inspectMcpProcesses({ commands: [
...
test('doctor reports a nonexistent local MCP ahead as unverified without targeting or downgrading it', () => {
  const report = inspectMcpProcesses({ commands: [
    'npx -y --package=@getmarrow/mcp@3.9.90 marrow-mcp',
  ] });
  assert.equal(report.expected_version, '3.9.89');
  assert.deepEqual(report.stale_versions, []);
  assert.deepEqual(report.ahead_unverified_versions, ['3.9.90']);
  assert.equal(report.ahead_unverified, true);
  assert.equal(report.healthy, false);
  assert.equal(report.automatic_repair_suppressed, true);
  assert.equal(report.restart_required, false);
  assert.match(report.exact_fix, /npm view @getmarrow\/mcp@latest name version dist\.integrity dist\.tarball --json/);
  assert.doesNotMatch(report.exact_fix, /--package=@getmarrow\/mcp@3\.9\.(?:89|90).*setup/);
});

test('doctor treats stable newer minor and major MCP processes as unverified-ahead', () => {
  const report = inspectMcpProcesses({ commands: [
...
test('mixed stale and unverified-ahead MCP evidence fails closed before automatic repair', () => {
  ...
  assert.equal(report.automatic_repair_suppressed, true);
  assert.match(report.exact_fix, /official npm registry access/);
  assert.doesNotMatch(report.exact_fix, /marrow-mcp setup/);
});
...
test('verified registry latest retargets update and repair plans with an exact pin', () => {
  ...
  const target = resolveMcpTargetVersion({
    ...
    registryMetadata: {
      ...
      dist: {
        ...
        tarball: 'https://registry.npmjs.org/@getmarrow/mcp/-/mcp-3.9.90.tgz',
      },
    },
  });
  ...
  try {
    ...
    assert.match(generated, /@getmarrow\/mcp@3\.9\.90/);
...
test('latest-target resolution stays offline-safe and rejects unverified or incompatible registry claims', () => {
  assert.deepEqual(resolveMcpTargetVersion({ currentVersions: ['3.9.90'] }), {
    version: '3.9.89',
  ...
  assert.equal(resolveMcpTargetVersion({ currentVersions: ['4.0.0'] }).version, '3.9.89');
});
...
test('offline repair preserves unverified-ahead config and never propagates it into managed targets', async () => {
  ...
  const configBefore = JSON.stringify({
    mcpServers: { marrow: { command: 'npx', args: ['-y', '--package=@getmarrow/mcp@3.9.90', 'marrow-mcp'] } },
  });
  const hooksBefore = JSON.stringify({
    hooks: { PreToolUse: [{ matcher: 'Bash', hooks: [{ type: 'command', command: 'npx -y --package=@getmarrow/mcp@3.9.90 marrow-mcp codex-pre-action-hook' }] }] },
  });
  fs.writeFileSync(path.join(root, 'package.json'), '{}\n');
  ...
  fs.writeFileSync(codexHooksPath, hooksBefore);
  const originalHome = process.env.HOME;
  process.env.HOME = home;
  try {
    const unsafeDirectPlan = buildPlan(detectEnvironment(root, { HOME: home }), {
      mode: 'mcp',
      agentId: 'unverified-ahead-direct-target',
      baseUrl: 'https://api.getmarrow.ai',
      toolProfile: resolveToolProfile(undefined),
      mcpTargetVersion: '3.9.90',
    });
    assert.equal(unsafeDirectPlan.mcp_target_version, '3.9.89');
    ...
    assert.equal(forgedRegistryPlan.mcp_target_version, '3.9.89');
    ...
    const report = await install({
      ...
      agentId: '',
      controller: false,
      processCommands: ['npx -y --package=@getmarrow/mcp@3.9.90 marrow-mcp'],
    ...
    assert.match(instructions, /@getmarrow\/mcp@3\.9\.89/);
    ...
    assert.deepEqual(suppressed.ahead_unverified_versions, ['3.9.90']);
    ...
    assert.deepEqual(report.doctor.ahead_unverified_versions, ['3.9.90']);
    ...
    assert.equal(report.adapterProvenance.mcp.version, '3.9.89');
...
test('update preserves stable newer minor and major MCP surfaces while suppressing automatic repair', async () => {
  const root = tempDir();
  const home = tempDir();
  const configPath = path.join(root, '.mcp.json');
  const codexHooksPath = path.join(root, '.codex', 'hooks.json');
  const configBefore = JSON.stringify({
    mcpServers: { marrow: { command: 'npx', args: ['-y', '--package=@getmarrow/mcp@3.10.0', 'marrow-mcp'] } },
  });
  const hooksBefore = JSON.stringify({
    hooks: { PreToolUse: [{ hooks: [{ type: 'command', command: 'npx -y --package=@getmarrow/mcp@4.0.0 marrow-mcp codex-pre-action-hook' }] }] },
  });
  fs.writeFileSync(path.join(root, 'package.json'), '{}\n');
  fs.writeFileSync(path.join(root, 'AGENTS.md'), '# Owner instructions\n');
  fs.writeFileSync(configPath, configBefore);
  ...
  try {
    const report = await install({
      ...
      apiKey: '',
      baseUrl: 'https://api.getmarrow.ai',
      agentId: 'cross-boundary-ahead',
      controller: false,
      processCommands: [
        'npx -y --package=@getmarrow/mcp@3.10.0 marrow-mcp',
        'npx -y --package=@getmarrow/mcp@4.0.0 marrow-mcp',
      ],
      mcpConfigPaths: [configPath, codexHooksPath],
      mcpRegistryMetadata: null,
    });

    assert.equal(fs.readFileSync(configPath, 'utf8'), configBefore);
    assert.equal(fs.readFileSync(codexHooksPath, 'utf8'), hooksBefore);
    assert.deepEqual(report.doctor.ahead_unverified_versions, ['3.10.0', '4.0.0']);
    assert.equal(report.doctor.automatic_mcp_repair_suppressed, true);
    assert.equal(report.doctor.restart_required, false);
    for (const label of ['Project MCP server config', 'Codex native hooks']) {
      const change = report.changes.find((entry) => entry.label === label);
      assert.equal(change.automatic_repair_suppressed, true, label);
      assert.equal(change.applied, false, label);
    }
    assert.match(report.doctor.recommendedFix, /official npm registry access/);
    assert.doesNotMatch(report.doctor.recommendedFix, /@getmarrow\/install@latest update|marrow-mcp setup/);
  } finally {
    if (originalHome === undefined) delete process.env.HOME;
    else process.env.HOME = originalHome;
    fs.rmSync(root, { recursive: true, force: true });
...
test('doctor uses verified registry evidence without emitting a downgrade recommendation', async () => {
  const root = tempDir();
  const configPath = path.join(root, '.mcp.json');
  const registryIntegrity = ADAPTER_PROVENANCE.mcp.integrity;
  fs.writeFileSync(path.join(root, 'package.json'), '{}\n');
  fs.writeFileSync(configPath, JSON.stringify({
    mcpServers: { marrow: { command: 'npx', args: ['-y', '--package=@getmarrow/mcp@3.9.79', 'marrow-mcp'] } },
  }));
  try {
    const report = await install({
      cwd: root,
      ...
      agentId: '',
      controller: false,
      processCommands: ['npx -y --package=@getmarrow/mcp@3.9.79 marrow-mcp'],
      mcpConfigPaths: [configPath],
      mcpRegistryMetadata: {
        name: '@getmarrow/mcp',
        version: '3.9.89',
        dist: {
          integrity: registryIntegrity,
          tarball: 'https://registry.npmjs.org/@getmarrow/mcp/-/mcp-3.9.89.tgz',
        },
      },
    });
    assert.equal(report.doctor.mcpProcesses.expected_version, '3.9.89');
    assert.equal(report.doctor.mcpConfigurations.expected_version, '3.9.89');
    ...
    assert.equal(fs.readFileSync(configPath, 'utf8').includes('3.9.79'), true);
    let output = '';
    ...
    assert.equal((output.match(/npx -y @getmarrow\/install@latest update/g) || []).length, 1);
    assert.equal((output.match(/After the update completes, restart the detected owning harnesses once\./g) || []).length, 1);
    assert.equal((output.match(/npx -y @getmarrow\/install@latest doctor --self-test/g) || []).length, 1);
    assert.doesNotMatch(output, /marrow-mcp setup/);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});

test('one update synchronizes mixed managed MCP surfaces with one restart and one doctor verification', async () => {
  const root = tempDir();
  const paths = {
    ...
    grok: path.join(root, '.grok', 'hooks', 'marrow.json'),
  };
  fs.writeFileSync(path.join(root, 'package.json'), '{}\n');
  for (const filePath of [paths.claude, paths.codex, paths.grok]) {
    fs.mkdirSync(path.dirname(filePath), { recursive: true });
  }
  fs.chmodSync(path.join(root, '.grok'), 0o700);
  fs.chmodSync(path.join(root, '.grok', 'hooks'), 0o700);
  fs.writeFileSync(paths.agents, `# Owner retained instructions
Historical owner example: \`npx -y --package=@getmarrow/mcp@3.9.79 marrow-mcp\`.

<!-- marrow:passive-start -->
- Use MCP: \`npx -y --package=@getmarrow/mcp@3.9.79 marrow-mcp setup\`.
<!-- marrow:passive-end -->
`);
  fs.writeFileSync(paths.mcp, JSON.stringify({
    owner: { retained: true },
    mcpServers: { marrow: { command: 'npx', args: ['-y', '--package=@getmarrow/mcp@3.9.79', 'marrow-mcp'] } },
  }, null, 2) + '\n');
  fs.writeFileSync(paths.claude, JSON.stringify({
    owner: { retained: true },
    hooks: { PreToolUse: [{ hooks: [{ type: 'command', command: 'npx -y --package=@getmarrow/mcp@3.9.89 marrow-mcp pre-action-hook' }] }] },
  }, null, 2) + '\n');
  fs.writeFileSync(paths.codex, JSON.stringify({
    owner: { retained: true },
    hooks: { PreToolUse: [{ hooks: [{ type: 'command', command: 'npx -y --package=@getmarrow/mcp@3.9.79 marrow-mcp codex-pre-action-hook' }] }] },
  }, null, 2) + '\n');
  fs.writeFileSync(paths.grok, JSON.stringify({
    enabled: false,
    owner: { retained: true },
    hooks: { PreToolUse: [{ hooks: [{ type: 'command', command: 'npx -y --package=@getmarrow/mcp@3.9.79 marrow-mcp grok-pre-action-hook', enabled: false }] }] },
  }, null, 2) + '\n', { mode: 0o600 });

  const configurationPaths = Object.values(paths);
  ...
  assert.deepEqual(before.configured_versions, ['3.9.79', '3.9.89']);
  ...
  const registryFetch = async () => {
    registryFetches += 1;
    return { ok: true, json: async () => registryMetadata };
  };
  const originalHome = process.env.HOME;
  ...
  try {
    ...
    assert.deepEqual(report.doctor.mcpConfigurations.configured_versions, ['3.9.89']);
    ...
    assert.equal(report.doctor.mcp_update_required, false);
    assert.match(report.doctor.recommendedFix, /running Marrow MCP processes remain unchanged until that restart/i);
    assert.doesNotMatch(JSON.stringify(report.doctor), /@getmarrow\/install@latest update/);
    assert.equal(report.harnessReload.required, true);
    assert.equal(report.harnessReload.live_in_this_process, false);
    const agents = fs.readFileSync(paths.agents, 'utf8');
    assert.match(agents, /Historical owner example: .*@getmarrow\/mcp@3\.9\.79/);
    const managedAgentsBlock = agents.slice(
      agents.indexOf('<!-- marrow:passive-start -->'),
      agents.indexOf('<!-- marrow:passive-end -->') + '<!-- marrow:passive-end -->'.length,
    );
    assert.match(managedAgentsBlock, /@getmarrow\/mcp@3\.9\.89/);
    ...
    for (const filePath of [paths.mcp, paths.claude, paths.codex, paths.grok]) {
      ...
      assert.match(contents, /@getmarrow\/mcp@3\.9\.89/, filePath);
    ...
    const grok = JSON.parse(fs.readFileSync(paths.grok, 'utf8'));
    assert.equal(grok.enabled, false);
    assert.equal(grok.hooks.PreToolUse.at(-1).hooks[0].enabled, false);
    ...
    const second = await install({
      ...options,
      processCommands: ['npx -y --package=@getmarrow/mcp@3.9.89 marrow-mcp'],
    });
    ...
    assert.deepEqual(second.doctor.mcpConfigurations.configured_versions, ['3.9.89']);
...
function writeSdkLock(root, declaredSpec = '^3.7.63') {
  fs.writeFileSync(path.join(root, 'package-lock.json'), JSON.stringify({
    ...
    packages: {
      ...
      'node_modules/@getmarrow/sdk': {
        version: '3.7.63',
        resolved: 'https://registry.npmjs.org/@getmarrow/sdk/-/sdk-3.7.63.tgz',
...
test('parseArgs makes only the bare install path default-on while preserving explicit controls', () => {
  ...
  assert.deepEqual(defaults.toolProfile, {
...
test('tool profile parsing preserves explicit profiles and rejects invalid values with bounded repair', () => {
  assert.deepEqual(resolveToolProfile(undefined), {
    configured_profile: 'unset',
    effective_profile: 'primary',
    expected_visible_count: 17,
  });
  ...
  assert.deepEqual(resolveToolProfile('core'), {
    configured_profile: 'core',
    effective_profile: 'core',
    expected_visible_count: 7,
  });
  assert.deepEqual(resolveToolProfile('full'), {
    configured_profile: 'full',
    effective_profile: 'full',
    expected_visible_count: null,
  });
  assert.equal(parseArgs([], { MARROW_TOOL_PROFILE: 'core' }).toolProfile.configured_profile, 'core');
  assert.equal(parseArgs([], { MARROW_TOOL_PROFILE: 'full' }).toolProfile.effective_profile, 'full');
  assert.throws(
    () => parseArgs([], { MARROW_TOOL_PROFILE: 'everything' }),
...
function primaryAvailability(evidenceState = 'available') {
  const tools = PRIMARY_TOOL_NAMES.map((name) => {
    const gated = name === 'marrow_workflow_gate' || name === 'marrow_fleet_lessons';
    return {
      name,
      state: gated ? 'upgrade_required' : 'entitled',
      always_available: !gated,
      plan_feature: gated ? (name === 'marrow_workflow_gate' ? 'pre_action_risk_gates' : 'fleet_learning') : null,
      minimum_plan: gated ? 'team' : null,
  ...
  return {
    ...
    counts: { total: 17, entitled: 15, upgrade_required: 2 },
    tools,
  };
}

test('self-test profile report trusts only matching reloaded MCP visibility and non-authorizing backend evidence', () => {
  const report = buildMcpToolProfileReport(
    resolveToolProfile(undefined),
    {
      configured_profile: 'unset',
      effective_profile: 'primary',
      visible_tool_count: 17,
      visible_tool_names: [...PRIMARY_TOOL_NAMES],
      local_visibility_grants_entitlement: false,
      backend_entitlement_projection: {
        evidence_state: 'unavailable',
        source: 'backend_projection_not_provided',
  ...
  assert.equal(report.configured_profile, 'unset');
  assert.equal(report.effective_profile, 'primary');
  assert.equal(report.expected_visible_count, 17);
  assert.equal(report.visible_tool_count, 17);
  assert.deepEqual(report.visible_tool_names, PRIMARY_TOOL_NAMES);
  assert.equal(report.visibility_live, true);
  assert.equal(report.reload_required, false);
  assert.equal(report.local_visibility_grants_entitlement, false);
  assert.equal(report.backend_entitlement_projection.evidence_state, 'available');
  assert.equal(report.backend_entitlement_projection.source, 'authenticated_backend');
  assert.equal(report.backend_entitlement_projection.authorizes_calls, false);
  assert.deepEqual(report.backend_entitlement_projection.primary_tool_availability.counts, {
    total: 17,
    entitled: 15,
    upgrade_required: 2,
  });
...
test('non-reloaded and unavailable profile evidence stays not-live and non-authorizing', () => {
  const cold = buildMcpToolProfileReport(
    resolveToolProfile(undefined),
    {
      configured_profile: 'core',
      effective_profile: 'core',
      visible_tool_count: 7,
      visible_tool_names: Array.from({ length: 7 }, (_, index) => `core-tool-${index}`),
      local_visibility_grants_entitlement: false,
    },
    primaryAvailability('unavailable'),
  );

  assert.equal(cold.visible_tool_count, null);
  assert.deepEqual(cold.visible_tool_names, []);
  assert.equal(cold.visibility_live, false);
  assert.equal(cold.reload_required, true);
  assert.equal(cold.backend_entitlement_projection.evidence_state, 'unavailable');
  assert.equal(cold.backend_entitlement_projection.authorizes_calls, false);

  const fullNames = Array.from({ length: 58 }, (_, index) => `full-tool-${index}`);
  const full = buildMcpToolProfileReport(resolveToolProfile('full'), {
    configured_profile: 'full',
    effective_profile: 'full',
    visible_tool_count: fullNames.length,
    visible_tool_names: fullNames,
    local_visibility_grants_entitlement: false,
  });
  assert.equal(full.expected_visible_count, 58);
  assert.equal(full.visible_tool_count, 58);
  assert.equal(full.visibility_live, true);
});

test('ordinary MCP setup leaves profile unset while explicit core/full profiles are preserved', () => {
  const createPlan = (root, toolProfile) => {
    fs.writeFileSync(path.join(root, 'package.json'), '{}');
    fs.writeFileSync(path.join(root, '.mcp.json'), '{}');
    return buildPlan(detectEnvironment(root, { HOME: root }), {
      mode: 'mcp',
  ...
  const ordinary = tempDir();
  applyPlan(createPlan(ordinary, resolveToolProfile(undefined)), { yes: true, dryRun: false, doctor: false });
  let config = JSON.parse(fs.readFileSync(path.join(ordinary, '.mcp.json'), 'utf8'));
  assert.equal('MARROW_TOOL_PROFILE' in config.mcpServers.marrow.env, false);

  const core = tempDir();
  applyPlan(createPlan(core, resolveToolProfile('core')), { yes: true, dryRun: false, doctor: false });
  config = JSON.parse(fs.readFileSync(path.join(core, '.mcp.json'), 'utf8'));
  assert.equal(config.mcpServers.marrow.env.MARROW_TOOL_PROFILE, 'core');

  const full = tempDir();
  fs.writeFileSync(path.join(full, 'package.json'), '{}');
  fs.writeFileSync(path.join(full, '.mcp.json'), JSON.stringify({
    mcpServers: { marrow: { env: { MARROW_TOOL_PROFILE: 'full' } } },
  }));
  const fullPlan = buildPlan(detectEnvironment(full, { HOME: full }), {
    mode: 'mcp',
    agentId: 'agent-profile-test',
    baseUrl: 'https://api.example.test',
    toolProfile: resolveToolProfile(undefined),
  });
  applyPlan(fullPlan, { yes: true, dryRun: false, doctor: false });
  config = JSON.parse(fs.readFileSync(path.join(full, '.mcp.json'), 'utf8'));
  assert.equal(config.mcpServers.marrow.env.MARROW_TOOL_PROFILE, 'full');
});
...
test('invalid stored MCP profile fails closed before configuration can broaden', () => {
  for (const invalidProfile of ['everything', '', '   ', ' core ']) {
    ...
    const plan = buildPlan(detectEnvironment(dir, { HOME: dir }), {
      mode: 'mcp',
      agentId: 'agent-profile-test',
      baseUrl: 'https://api.example.test',
      toolProfile: resolveToolProfile(undefined),
...
test('SDK detection accepts the exact npm-verified 3.7.63 package and rejects a different integrity', () => {
  const dir = tempDir();
  const moduleDir = path.join(dir, 'node_modules', '@getmarrow', 'sdk');
  fs.writeFileSync(path.join(dir, 'package.json'), JSON.stringify({ dependencies: { '@getmarrow/sdk': '3.7.63' } }));
  fs.mkdirSync(moduleDir, { recursive: true });
  fs.writeFileSync(path.join(moduleDir, 'package.json'), JSON.stringify({ name: '@getmarrow/sdk', version: '3.7.63' }));
  writeSdkLock(dir, '3.7.63');
  ...
  assert.equal(report.install_command, 'npm install @getmarrow/sdk@3.7.63');
});

test('SDK doctor preserves an installed stable version ahead of its sealed target', () => {
  const dir = tempDir();
  const moduleDir = path.join(dir, 'node_modules', '@getmarrow', 'sdk');
  fs.writeFileSync(path.join(dir, 'package.json'), JSON.stringify({ dependencies: { '@getmarrow/sdk': '3.8.0' } }));
  fs.mkdirSync(moduleDir, { recursive: true });
  fs.writeFileSync(path.join(moduleDir, 'package.json'), JSON.stringify({ name: '@getmarrow/sdk', version: '3.8.0' }));
  fs.writeFileSync(path.join(dir, 'package-lock.json'), JSON.stringify({
    lockfileVersion: 3,
    packages: {
      '': { dependencies: { '@getmarrow/sdk': '3.8.0' } },
      'node_modules/@getmarrow/sdk': {
        version: '3.8.0',
        resolved: 'https://registry.npmjs.org/@getmarrow/sdk/-/sdk-3.8.0.tgz',
...
test('generated passive instructions pin the supported MCP release', () => {
  ...
  assert.match(instructions, /npx -y --package=@getmarrow\/mcp@3\.9\.89 marrow-mcp setup/);
...
test('generated passive runtime prefers current process identity and falls back to installed identity', async () => {
  ...
  fs.writeFileSync(path.join(dir, '.mcp.json'), JSON.stringify({
    mcpServers: {
      marrow: {
        command: 'npx',
        args: ['-y', '--package=@getmarrow/mcp@3.9.79', 'marrow-mcp'],
        env: {
...
test('install reports missing SDK dependency for passive runtime projects', async () => {
  ...
  assert.equal(report.sdkDependency.install_command, 'npm install @getmarrow/sdk@3.7.63');
  ...
  assert.equal(sdk.installed_version, '3.7.63');
...
test('repair mode writes config and reports self-test remediation state', async () => {
  ...
  assert.match(output, /configured tool profile: unset/);
  assert.match(output, /effective tool profile: primary/);
  assert.match(output, /expected visible tools: 17/);
  assert.match(output, /actual visible tools: unavailable until process reload/);
  assert.match(output, /visible tool names: unavailable until process reload/);
  assert.match(output, /backend-projected entitlements: unavailable.*non-authorizing/);
});
...
test('self-test returns first five-minute value signal and proof', async () => {
  ...
  global.fetch = async (url, request = {}) => {
    ...
    if (href.endsWith('/v1/agent/context')) {
      return new Response(JSON.stringify({ data: {
        primary_tool_availability: primaryAvailability('available'),
      } }), { status: 200 });
    }
    if (href.endsWith('/v1/agent/runtime')) {
      return new Response(JSON.stringify({ data: {
  ...
  try {
    ...
    assert.equal(requestHeaders['x-marrow-package-version'], '0.1.58');
    assert.equal(requestHeaders['x-marrow-install-version'], '0.1.58');
    assert.equal(requestHeaders['x-marrow-sdk-version'], '3.7.63');
    assert.equal(requestHeaders['x-marrow-mcp-version'], '3.9.89');
    ...
    assert.equal(result.mcp_tool_profile.visible_tool_count, 17);
    assert.equal(result.mcp_tool_profile.visibility_live, true);
    assert.equal(result.mcp_tool_profile.backend_entitlement_projection.evidence_state, 'available');
    assert.equal(result.mcp_tool_profile.backend_entitlement_projection.authorizes_calls, false);
    assert.equal(result.mcp_tool_profile.backend_entitlement_projection.primary_tool_availability.counts.entitled, 15);
    assert.equal(result.mcp_tool_profile.backend_entitlement_projection.primary_tool_availability.counts.upgrade_required, 2);
