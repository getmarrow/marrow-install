...
test('local integration registry covers major harnesses and model CLIs', () => {
  ...
  assert.ok(mcpSetupCommands.every((command) => command.includes('--package=@getmarrow/mcp@3.9.92')));
...
test('governed runner attaches stable client attribution from env and CLI', () => {
  ...
  try {
    ...
    assert.equal(headers(envParsed.options)['X-Marrow-Package-Version'], '0.1.60');
    assert.equal(headers(envParsed.options)['X-Marrow-Install-Version'], '0.1.60');
    assert.equal(headers(envParsed.options)['X-Marrow-SDK-Version'], '3.7.63');
    assert.equal(headers(envParsed.options)['X-Marrow-MCP-Version'], '3.9.92');
