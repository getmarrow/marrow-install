...
test('governed runner attaches stable client attribution from env and CLI', () => {
  ...
  try {
    ...
    assert.equal(headers(envParsed.options)['X-Marrow-Package-Version'], '0.1.42');
...
test('fleet panel summarizes operator-critical account state', () => {
  ...
  assert.match(panel, /Marrow client update: recommended; installed=0\.1\.35; latest=0\.1\.42; operator approval required/);
...
test('status panel makes an available update actionable without silently applying it', () => {
  ...
  assert.match(panel, /Client update: recommended; installed=0\.1\.35; latest=0\.1\.42/);
